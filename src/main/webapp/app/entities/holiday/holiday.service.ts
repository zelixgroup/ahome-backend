...
export class HolidayService {
  ...
  protected convertDateFromClient(holiday: IHoliday): IHoliday {
    const copy: IHoliday = Object.assign({}, holiday, {
      correspondingDate:
        holiday.correspondingDate && holiday.correspondingDate.isValid() ? holiday.correspondingDate.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.correspondingDate = res.body.correspondingDate ? moment(res.body.correspondingDate) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((holiday: IHoliday) => {
        holiday.correspondingDate = holiday.correspondingDate ? moment(holiday.correspondingDate) : undefined;
      });
    }
    return res;
  }
