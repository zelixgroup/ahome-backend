...
export class WorkCategoryComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.workCategoryService
      ...
      .subscribe(
        (res: HttpResponse<IWorkCategory[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  registerChangeInWorkCategories(): void {
    this.eventSubscriber = this.eventManager.subscribe('workCategoryListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IWorkCategory[] | null, headers: HttpHeaders, page: number): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.router.navigate(['/work-category'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
